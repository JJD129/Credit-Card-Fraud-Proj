...
prop.table(table(creditcardFraud$class)) #looking at proportion of yes no
...
train <- createDataPartition(creditcardFraud$class,
                           p=.70, # setting 70% training, remaining 30% test
                           times = 1, # sample once
                           list = F) # not a list
train.orig <- creditcardFraud[train,] #training dataset
test <- creditcardFraud[-train,] # selecting remaining 30% of the train dataset
...
ctrl <- trainControl(method = "cv", # crossvalid
                     number = 10, # 10X
                     classProbs = TRUE, 
...
#B. NB - Assign class to probabilities
nb_adas_test <- factor(ifelse(nb_adas_pred$yes>0.5,'yes','no'))
...
#B. LDA - Assign class to probabilities
lda_adas_test <- factor(ifelse(lda_adas_pred$yes>0.5,'yes','no'))
